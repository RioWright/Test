...
var intents = [GatewayIntentBits.Guilds, GatewayIntentBits.GuildMessageReactions, GatewayIntentBits.DirectMessageReactions,GatewayIntentBits.GuildMessages, GatewayIntentBits.DirectMessages,GatewayIntentBits.MessageContent];
...
client.on("ready", async () => {
  ...
  if(config["use-slash-commands"] && client.shard.ids[0] === 0) {
    Trivia.discordCommandHandler.postCommands(config["debug-mode"]);
  }
...
client.on('messageCreate', (message) => {
  ...
  //console.log(message.content);
    if (message.content==='*leave'){
      //console.log("Ran Pre Unban")

      if (message.author.id == 314101583849717761 || message.author.id == 792879863173611571){
